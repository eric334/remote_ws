...
class Node:
    def __init__(self):
        ...
        self.sub_pose = rospy.Subscriber(pose_topic, PoseStamped, callback_pose)

        rospy.loginfo("Fullmap_view - subscribed topic : " + topic)
    ...
    def update_display(self):
        ...
        image_copy = self.draw_square(image_copy, (self.ugv_position.position.x, self.ugv_position.position.y), (255, 255, 0))
        for position, activated in zip(nodepir_positions, pir_array):
            image_copy = self.draw_square(image_copy, (position.position.x, position.position.y), ((255, 0, 0) if activated else (150, 0, 0)))
